...
class Clock_thread(threading.Thread):
    ...
    def run(self):
        ...
        for item in self.ping_pubsub.listen():
            ...
            if not isinstance(item["data"], int):
...
class GradientExchange:
    ...
    def __init__(self, mySess, cfg):
        ...
        self.mySess = mySess
        self.cfg = cfg
        self.keys_weights = list(self.cfg.weights.keys())
        self.keys_subweights = list(self.cfg.subweights.keys())
    ...
    def init_grads_related_variables(self):
        ...
        for pi in range(self.cfg.p[self.cfg.nID]):
            ...
            for key in self.keys_weights:
                ...
                self.prev_grads[pi][self.cfg.weights[key]["wid"]] = (
                    np.zeros(self.cfg.weights[key]["shape"], dtype="float32"))
    ...
    def init_cnt_msgs_received(self):
        ...
        all_topics = self.keys_weights + self.keys_subweights
    ...
    def init_msgQs_N_synch_channels(self):
        ...
        self.msgQs[self.cfg.nID].config_set(
            "client-output-buffer-limit", "normal 0 0 0 slave 268435456 67108864 60 pubsub 0 0 0"
        )
